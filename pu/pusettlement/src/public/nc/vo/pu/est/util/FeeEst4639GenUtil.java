 ...
 * @time 2010-8-19 下午04:36:42
 */
public class FeeEst4639GenUtil {

  /** 将I9的来源恢复为入库单(消耗汇总)费用暂估分摊明细中记录的传IA的标识PK **/
  public static void restoreP4639SrcBID(P4639ItemVO[] i9items,
      FeeEstDistVO[] fdVos) {
    Map<String, FeeEstDistVO> fdMap = CirVOUtil.createKeyVOMap(fdVos);
    for (P4639ItemVO item : i9items) {
      String pk_fd = item.getCsrcbid();
      FeeEst4639GenUtil.checkI9SrcInfo(fdMap, pk_fd);
  ...
  /** VO交换完成后设置I9的调整对象 **/
  public static void setAdjustInfo(P4639ItemVO[] p436items, FeeEstDistVO[] fdVos) {
    Map<String, FeeEstDistVO> fdMap = CirVOUtil.createKeyVOMap(fdVos);
    for (P4639ItemVO itemVO : p436items) {
   ...
   * @param mny
   */
  public static void setCostFactor(P4639ItemVO i9item, int cfnum, UFDouble mny) {
    if (1 == cfnum) {
   ...
   * @param fdVos
   */
  public static void setCostFactor(P4639ItemVO[] items, FeeEstVO[] fees,
      FeeEstDistVO[] fdVos) {
    Map<String, FeeEstVO> feeMap = CirVOUtil.createKeyVOMap(fees);
    Map<String, FeeEstDistVO> fdMap = CirVOUtil.createKeyVOMap(fdVos);
    for (P4639ItemVO i9item : items) {
      // 实际为暂估分摊明细ID
      ...
      FeeEst4639GenUtil.setCostFactor(i9item, cfnum, mny);
    }
  }
