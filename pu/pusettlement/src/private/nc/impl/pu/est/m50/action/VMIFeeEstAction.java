 ...
 * @time 2010-5-25 下午03:55:07
 */
public class VMIFeeEstAction {
  public void estimate(VmiEstVO[] vos) {
    if (ArrayUtils.isEmpty(vos)) {
      return;
    }
    EstVOTransferTool<VmiEstVO> tool =
        new EstVOTransferTool<VmiEstVO>(QueryEstType.FEE_EST, vos,
            VmiFIFeeVO.class);
    VmiEstVO[] origVos = tool.getOrigVos();
    CompareAroundProcesser<VmiEstVO> prcr =
        new CompareAroundProcesser<VmiEstVO>(VMIEstPluginPoint.FEE_EST_ACTION);
    this.addRule(prcr);
    prcr.before(vos, origVos);
    new VMIEstBP().feeEstimate(vos, origVos);
