...
import nc.bs.pu.m20.maintain.rule.close.OpenBudgetCtrlRule;
import nc.bs.pu.m20.maintain.rule.close.SetOpenStatus;
 ...
 * @since 6.0
 * @author GGR
 * @time 2010-4-15 上午10:43:17
 */
public class PraybillOpenBP {

  public PraybillVO[] openBill(PraybillVO[] updateVos, PraybillVO[] originalVos) {
    CompareAroundProcesser<PraybillVO> processer =
        new CompareAroundProcesser<PraybillVO>(PraybillPluginPoint.OPEN_BILL);
  ...
  private void addAfterRule(CompareAroundProcesser<PraybillVO> processer) {
    // 回写采购计划
    processer.addAfterRule(new OpenBudgetCtrlRule());
    // 保存后可用量处理
  ...
  private void addBeforeRule(CompareAroundProcesser<PraybillVO> processer) {
    ...
    // 状态设置
    processer.addBeforeRule(new SetOpenStatus());
    // 保存前可用量处理
