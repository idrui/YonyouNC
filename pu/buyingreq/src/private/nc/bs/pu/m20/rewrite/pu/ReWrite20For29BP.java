...
public class ReWrite20For29BP {
   ...
   * @since 6.0
   * @author gaogr
   * @time 2010-8-18 上午10:57:02
   */
  public void backWriteGenNum(M29WriteBackVO[] vos) {
  ...
  private void upAdd(List<List<Object>> adddata) {
    if (adddata.size() > 0) {
      String addsql =
          "UPDATE po_praybill_b set nquotebill = coalesce(nquotebill,0) + 1  WHERE pk_praybill_b = ? ";
