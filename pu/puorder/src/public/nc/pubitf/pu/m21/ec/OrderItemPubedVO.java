...
public class OrderItemPubedVO extends AbstractDataView {
  ...
  public static class OrderItemPubedVOMeta extends DataViewMeta {
    ...
    private void init() {
      /**
       * 订单主键、订单子表主键、合同号、物料id、自由辅助属性id、批次号、
       * 单位id、订单数量、确认数量、订单单价、含税单价、折扣率(%)、净含税单
       * 价、价税合计、税率（%）、税额、币种id、订单计划到货日期、确认计划到
       * 货日期、运输方式id、收货地址id、生产厂商id、质量等级id
       * 原始订单数量、换算率、辅单位、TS、辅数量、主单位、主含税单价、主数量
