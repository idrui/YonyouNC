...
/**
 * @since 6.0
 * @version 2011-5-16 上午11:12:18
 * @author wuxla
 */

public class OrderPublishedViewVO extends AbstractDataView {

  private static final long serialVersionUID = 2374452194278693846L;

  public static class OrderPublishedViewVOMeta extends DataViewMeta {
    public OrderPublishedViewVOMeta() {
      init();
    }

    private void init() {
      // 订单主键、采购订单号、采购组织id、采购部门id、采购员id、订单日期
      // 响应状态(整数)、订单状态(整数)、供应商发货地址id、发票方id、付款协议id、
      // 备注,运输方式,冻结状态
  ...
  public IDataViewMeta getMetaData() {
    return DataViewMetaFactory.getInstance().getDataViewMeta(
        OrderPublishedViewVOMeta.class);
  }
