...
/**
 * @since 6.0
 * @version 2011-3-19 下午12:36:52
 * @author yinfy
 */

public abstract class PrayBillNumFetchDataProviderStrategy implements
  ...
  protected UFDouble[][] getReadyDataBatch(NtbParamVO[] param,
    ...
    // 取未关闭的请购单的数量
    UFDouble[][] part = dmo.getReadyDataBatch(param, PlBillBusiSysReg.NNUM);
    if (null != part) {
    ...
    // 取已关闭的请购单的累积订单数量
    part = orderasPray.getReadyDataBatch(param, PlBillBusiSysReg.NNUM);
    // 两个结果累加
    if (null != part) {
      for (int i = 0; i < result.length; i++) {
        if (null != part[i]) {
    ...
    part = ordercloseasPray.getReadyDataBatch(param, PlBillBusiSysReg.NNUM);
    // 两个结果累加
    if (null != part) {
      for (int i = 0; i < result.length; i++) {
        if (null != part[i]) {
