...
/**
 * @since 6.0
 * @version 2011-4-2 下午03:42:21
 * @author yinfy
 */

public class ArrivalAsPrayBillBatchFetcher extends OrderAsPrayBillBatchFetcher {
  @Override
  protected String getExecQuerySql(NtbParamVO seed, NtbParamVO[] params,
    ...
    if (PuBillBusiSysReg.NNUM.equals(type)) {
      select.append("select sum(ob.naccumarrvnum) ");
      tbbuilder.setStartIndex(1);
    }
    else if (PlBillBusiSysReg.NTAXMNY.equals(type)) {
      // wuxla、刘国强、王印芬、崔玲玲2012-8-10，确定使用主含税净价
      select
          .append("select b.pk_org,ob.corigcurrencyid,sum(coalesce(ob.norigtaxnetprice,0)*coalesce(ob.naccumarrvnum,0)) ");
    ...
    where.append(" and ob.barriveclose='" + UFBoolean.TRUE.toString() + "'");
