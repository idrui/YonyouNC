...
//				PraybillItemVO praybillItemVO = bvos[i];// 请购单字表
				...
				KycgjhxxjkHVO srchvo = (KycgjhxxjkHVO) bo.queryByPrimaryKey(
						KycgjhxxjkHVO.class, srcid);// 来源单据VO
				String wucode = srchvo.getCode();// 来源单物料编码
...
//				MeasdocVO.class, " name='" + srchvo.getCastunitid() + "'and code='" + srchvo.getCastunitid() + "'");
	...
	public String getMarbasclasscode(){
		...
		try {
			initVO = SysInitQuery.querySysinitVO(pk_org, "KSCGJH001");//获取业务参数方法（组织主键，参数编码）
		} catch (BusinessException e1) {
			Logger.error("矿山采购计划物料基本分类参数查询异常！", e1);
		}
