 ...
 * @time 2010-7-9 上午09:34:08
 */
public class OrderWriteBackForF1VerifyHandler implements IBusinessListener {

  @SuppressWarnings("unchecked")
  @Override
  public void doAction(IBusinessEvent event) throws BusinessException {
    try {
      if (ArapBillDealVOConsts.TYPE_VERIFY_AFTER.equals(event.getEventType())) {
        ...
        OrderWriteBackForF1Util f1util = OrderWriteBackForF1Util.getInstance();

        if (value.getClass().isArray()) {
          Object[] obj = (Object[]) value;
          AggverifyVO[] aggVOs = new AggverifyVO[obj.length];
          System.arraycopy(obj, 0, aggVOs, 0, obj.length);
          payableBillItems = f1util.getPayableBillItems(aggVOs);
          detailVOMap = f1util.getDetailVOMap(aggVOs);
        }
        else if (value.getClass().getName().equals(AggverifyVO.class.getName())) {
          AggverifyVO aggVO = (AggverifyVO) value;
          payableBillItems = f1util.getPayableBillItems(new AggverifyVO[] {
            aggVO
          });
          detailVOMap = f1util.getDetailVOMap(new AggverifyVO[] {
            aggVO
          });
        ...
        if (ArrayUtils.isEmpty(payableBillItems)
            && ArrayUtils.isEmpty(payBillItems) || null == detailVOMap) {
          return;
        }
        ...
        IOrderWriteBackParaForF1 para = new IOrderWriteBackParaForF1() {
          ...
          public UFDouble getDiffMny() {
            if (rbverify
                && MathTool.compareTo(detailVO.getLocal_money_cr(),
                    UFDouble.ZERO_DBL) == 0) {
              return detailVO.getLocal_money_de();
            }

            return detailVO.getLocal_money_cr();
          }
          ...
          public UFDouble getDiffOrgMny() {
            if (rbverify
              ...
              return detailVO.getMoney_de();
            }

            return detailVO.getMoney_cr();
          }
