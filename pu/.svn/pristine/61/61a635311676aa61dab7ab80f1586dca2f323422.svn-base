 ...
 * @time 2010-9-15 上午09:08:52
 */
public class AccStoreNumChkFor4TRule implements IRule<OrderViewVO> {
  private IOrderWriteBackPara[] wbVos;

  public AccStoreNumChkFor4TRule(IOrderWriteBackPara[] wbVos) {
    this.wbVos = wbVos;
  }
  ...
  private void checkAccNumber(OrderViewVO vo, IOrderWriteBackPara wbVo,
    ...
    try {
      ...
      if (MathTool.isDiffSign(orderNum, accNum)) {
        throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("4004030_0","04004030-0147")/*@res "存在订单累计入库数量与订单数量正负不一致的行，请检查！"*/);
      }
      ...
      // 累计到货(入库数量)大于订单容差数量
      if (MathTool.absCompareTo(accNum, toleranceNum) > 0) {
        if (ctrltype.not_save.equals(PO02)) {
          throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("4004030_0","04004030-0148")/*@res "存在期初暂估单数量超出订单行数量容差范围！"*/);
        }
        else if (ctrltype.ask.equals(PO02) && !wbVo.isNumUserConfirm()) {
          throw new AskNumException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("4004030_0","04004030-0149")/*@res "存在入库单收数量超出订单行数量容差范围,是否继续？"*/);
        }
      }
    }
    catch (Exception e) {
  ...
  private Map<String, UFDouble> getIntoleranceMap(OrderViewVO[] vos) {
