...
public class TotalValueHanlder implements TableModelListener {
  ...
  public void reCalAll() {
    ...
    for (String bodyName : this.getBodyHeadMap().keySet()) {
      allTotal.put(this.getBodyHeadMap().get(bodyName), new UFDouble(0, 0));
    }
    ...
    for (Entry<String, UFDouble> entry : allTotal.entrySet()) {
      ...
      this.getBillCardPanel().getHeadItem(headName)
          .setDecimalDigits(-total.getPower());
  ...
  public void tableChanged(TableModelEvent e) {
    ...
    if ((UIState.EDIT != this.getModel().getUiState())
        && (UIState.ADD != this.getModel().getUiState())) {
      return;
    }
    ...
    BillItem item = this.getBillModel().getBodyItems()[e.getColumn()];
  ...
  protected void calTotalValue(String bodyName, String headName) {
    ...
    int digit = null != total ? -total.getPower() : -1;
    if (-1 != digit) {
      this.getBillCardPanel().getHeadItem(headName).setDecimalDigits(digit);
    }
    this.getBillCardPanel().getHeadItem(headName).setValue(total);
  }
