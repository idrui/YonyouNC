...
import nc.bs.pu.m20.maintain.rule.close.OpenBill;
 ...
 */
public class PraybillOpenRowBP {

  public PraybillVO[] openBillRow(PraybillVO[] updateVos,
      PraybillVO[] originalVos) {
    CompareAroundProcesser<PraybillVO> processer =
        new CompareAroundProcesser<PraybillVO>(PraybillPluginPoint.OPEN_ROW);
  ...
  private void addAfterRule(CompareAroundProcesser<PraybillVO> processer) {
    ...
    // 行打开后自动打开
    processer.addAfterRule(new OpenBill());
    // 保存后可用量处理
