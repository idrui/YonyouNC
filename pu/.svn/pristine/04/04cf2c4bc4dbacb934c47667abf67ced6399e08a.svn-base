...
/**
 * 采购组织权限扩展类<br/>
 * 组织或集团级的汇总报表，汇总维度不包含组织或集团时，由于平台在默认的组织扩展类中会拼接组织SQL，导致SQL出错。
 * 此时可以将自由报表预置中的数据 -> 功能注册 -> 组织权限扩展类注册为此类，此类将会拼出与平台不同的SQL，回避错误。 <br/>
 * <br/>
 * 注：如在报表预置中包含主组织字段，则不能关联此扩展类，否则可能会出现问题。
 * 
 * @since 6.3
 * @version 2013-3-5 上午10:34:14
 * @author lixyp
 */
public class PuOrgAuthorityExt extends DefaultOrgAuthorityExt {

  @Override
  public FilterDescriptor getMainOrgDescriptor(IContext context, String areaPK,
      SmartModel smartModel, NODE_TYPE nodeType, Field groupField,
      boolean isGroupSet, Field orgFieldSet, Field orgField)
    ...
    if (NODE_TYPE.GROUP_NODE.equals(nodeType)) {
      // 采购目前没有集团级报表
      super.getMainOrgDescriptor(context, areaPK, smartModel, nodeType,
          groupField, isGroupSet, orgFieldSet, orgField);
    }
    // 如果节点类型是组织类型
      ...
      String[] orgPks =
          (String[]) context
              .getAttribute(FreePrivateContextKey.KEY_MAINORG_ORG);
      // 根据报表平台刘丽的建议，取到组织查询条件中第一个PK值（带单引号）
      String cond_org =
          (String) context.getAttribute(FreePrivateContextKey.KEY_USER_PK_ORG);
